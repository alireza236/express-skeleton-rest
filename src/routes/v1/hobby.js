...
module.exports = (HobbyService) => resource({
  ...
  load({query,params}, id, callback) {
    ...
    return HobbyService.find(id, option)
  ...
  async index({ user, query, params, payloads }, res) {
    try {
      //merge query & params
      let response = await HobbyService.findAll({
        user,
  ...
  async create({ user, body, params, payloads }, res) {
    try {
      let response = await HobbyService.create({
        user,
