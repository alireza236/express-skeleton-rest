...
class HobbyService {
  ...
  find(id, params = {}) {
    ...
    const rawAttrs = Object.keys(this.db.Hobby.rawAttributes) || [];
    ...
    try {
      return this.db.Hobby.findOne(option);
    } catch (err) {
      throw err;
    }
  }
  ...
  async findAll({ params }) {
    ...
    try {
      ...
      let collection = await this.db.Hobby.findAndCountAll(option);
  ...
  create({ params }) {
    try {
      return this.db.Hobby.create(params);
    } catch (err) {
      throw err;
    }
  }
